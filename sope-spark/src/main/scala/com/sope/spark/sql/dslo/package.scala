...
package object dslo {
  ...
  object Select {
    ...
    /**
     * Select columns from given dataframe. The Dataframe on which this function is called should contain all columns
     * from the
     ...
     * @param includeColumns Any columns to be included from the opposite side of join. Should not conflict with
     *                       aliased columns.
     * @param excludeColumns Any columns to be excluded from alias columns to selected
     * @return [[DFFunc]]
     */
    def apply(priorDF: DataFrame, alias: String, includeColumns: Seq[String] = Nil, excludeColumns: Seq[String] =
    Nil): DFFunc =
  ...
  */
  object Drop {
     ...
     * @return [[DFFunc]]
     */
    def apply(dropColumns: String*): DFFunc = (df: DataFrame) => df.drop(dropColumns: _*)
  }
  ...
  */
  object Transform {
     ...
     *
     * @param columnFunc column expression
     * @param columns    List of resultant column name and source column names/expressions to which the function is
     *                   to be applied
     * @return [[DFFunc]]
     */
  ...
  */
  object Aggregate {
     ...
     * @return [[DFFunc]]
     */
    def apply[T: ClassTag](aggregateExprs: Column*): DFFunc = (df: DataFrame) => df.agg(aggregateExprs.head,
      aggregateExprs.tail: _*)
  }
  ...
  */
  ...
  object DQCheck {
     ...
     * @return [[DFFunc]]
     */
    def apply[T: ClassTag](id: String, dqFunction: ColFunc, columns: Seq[String]): DFFunc = DQCheck(id, dqFunction,
      columns.map(col))
  }
